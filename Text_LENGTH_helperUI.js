...
(function () {
    ...
    // Autofocus management (moved from dashboard): focus the textarea on load
    // but avoid forcing the on-screen keyboard on mobile devices.
    ...
    // Render an accessible "no results" state
    window.renderNoWordsFound = function renderNoWordsFound() {
        if (wordLengthsEl) {
    ...
    // Finalize UI state after processing: show summaries, announce, and re-enable UI
    window.finalizeAndAnnounce = function finalizeAndAnnounce(totalWords, minLen, minWordsArr, maxLen, maxWordsArr) {
